...
export const PulsantiCalendarioPanel: React.FC<PulsantiCalendarioPanelProps> = ({ options, data, width, height, fieldConfig, id }) => {
  ...
  const handleButtonClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    let dataInizio
    let dataFine = currentDate.toISOString()
    switch (e.currentTarget.innerText) {
      case 'Ultima Settimana':
        dataInizio = new Date(new Date().setDate(new Date().getDate()-7)).toISOString()
        setClickedButton('week')
        break
      case 'Ultimo Mese':
        dataInizio = new Date(currentDate.setMonth(currentDate.getMonth() - 1)).toISOString()
        setClickedButton('month')
        break
      case 'Ultimo Anno':
        let currentYear = currentDate.getFullYear()
        dataInizio = new Date(currentYear, 0, 1).toISOString()
